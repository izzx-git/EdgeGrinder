CheckBackgrounds2 ;вызывается до того как нарисованы спрайты
; check if centre (horizontal length) of player bullet is over any background
;    ld a,(base_addr+1)
;    bit 6,a
;    jr z,ChkBG2Low
    ld hl,SpriteAddrHigh+154+6 ;адрес пули на экране, центр
;    jr ChkBG2SkipLow
;ChkBG2Low
...
; along with resetting the screen address list due to the background collision
    ld bc,(base_addr)
    ld hl,#3e80 ;640 ;адрес вне экрана
    add hl,bc
;    set 3,h
    ex de,hl
...
; set the high or low screen address pointer according to frame in use
 ...
 ;   bit 6,a
 ;   jr z,PlyCollLow
    ld ix,SpriteAddrHigh+132+6 ; top of centre for kill check ;адрес игрока на экране, центр
...
; but not both in one game frame
 ...
 ;   res 3,h
 ;   ld a,(hl)
	ld c,a ;запомним grinding для верхнего края
	;и повторим для нижнего
	ld hl,SpriteAddrHigh+132+20 ; sprite bottom for bottom grind check
    ld e,(hl)
    inc l
    ld d,(hl)
	ex de,hl
 ...
 ;   res 3,h
 ;   ld a,(hl)
	or c ;объединим с верхним результатом
    ret z
...
PlayColFatal ; also used by player sprite collision
    ...
    dec (hl)  ;вот здесь бесконечные жизни
	;nop
    ld a,(hl)
    or a
...
; need to continue
    ...
    add a,SpriteData+5-8 ;#60+5-8 ; and starts at #60, while b=1-6 so -8 is applied, +5 is animation entry
    ld l,a
