...
; need to prevent auto fire
    and a,16
auto_fire ;тут адрес включения авто огонь
...
;scankey
 ...
 ;   pop bc
  ...
  ;  out (c),c
	...
	ld a,0
	jr nz,scankey1
	ld a,4 ;установим бит 2 = Escape = пауза или выход, если удерживать после второго нажатия
scankey1
	ld (hl),a ;сохраним в буфере


;обработка кнопки счётчика кадров	
	ld b,#fb
	in a,(c)
	bit 3,a ;клавиша "R"
	jr nz,scankey2
	ld a,1	;включить счётчик кадров
	ld (FrameRate_Sw),a
scankey2



;обработка кнопки авто огонь
	ld b,#fd
	in a,(c)
	bit 3,a ;клавиша "F"
	jr nz,scankey3
	ld a,#18	;включить auto fire (ставим jr вместо jr z)
	ld (auto_fire),a
scankey3


	
;опрос остальных клавиш
	call KBDJOY
	ld hl,KBmatrixbuf+9
	ld (hl),a
    ret ; 64 - 2 + 10 * 23 = 292
