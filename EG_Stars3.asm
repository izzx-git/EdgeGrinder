scolorxor equ  5 ;цвет звёзд меняется по xor с этим числом
scolor equ 4 ;цвет звезд изначально 
...
;StarClearLow
 ;   ld hl,StarsLow
;StarClearSkipLow
    ld bc,#a*256+scolor;#a0a ; c = star byte
...
; that do not shift the screen precisely half a character
    ...
    ;dec a ; the scroll step is incremented after use in the interupt
 	;ld (scroll_step),a	
    bit 0,a ; so a dec is required to synchronise with R3 change 
    jr z,StarClearLp
    ld a,scolorxor ;42 ; the byte for light green star is #a, dark green is #20
    xor a,c
    ld c,a
;
StarClearLp
...
DontClearStar
    ...
    ld a,scolorxor ;42 ; otherwise invert the star byte in c
    xor a,c
    ld c,a
StarClrDontInvert
    djnz StarClearLp
...
; that do not shift the screen precisely half a character
    ld a,(scroll_step)
    ;dec a ; the scroll step is incremented after use in the interupt
    bit 1,a ; so a dec is required to synchronise with R3 change     ;тут можно регулировать частоту мерцания звёзд
    jr nz,StarPrnLp
    ld a,scolorxor ;42 ; the byte for light green star is #a, dark green is #20
    xor a,c
    ld c,a
