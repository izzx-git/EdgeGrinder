...
//Генератор кода для Edge Grinder ZS GMX, выводящего спрайт. На входе _линейные_ спрайты, на выходе код с данными.
namespace MakeGFXCode0
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            for (int i1 = 0; i1 < counter; i1++) //основной цикл по количеству спрайтов
            {
                ...
                for (int i0 = 0; i0 < 10; i0++) //основной цикл 10 пар строк
                {
                    ...
                    for (int i = 0; i < 6; i++) //одна строка 6 байт
                    {
                        ...
                        byte_in_s = byte_in[i + 6].ToString();
                        if (byte_in_s != "0")
                        {
                            byte_out += "\tld (hl)," + byte_in_s + "\r\n";
                        }
                ...
                if (i1 == 39) //запишем первый файл
                {
                    FileStream FS = new FileStream(file_out + "_0.asm", FileMode.Create);
                    StreamWriter SW = new StreamWriter(FS, Encoding.Default);
                    SW.WriteLine(byte_out); //запишем выходной файл
                    byte_out = "";//и очистим переменную
                    SW.Close();
                }
                if (i1 == 79) //запишем первый файл
                {
                    FileStream FS = new FileStream(file_out + "_1.asm", FileMode.Create);
                    StreamWriter SW = new StreamWriter(FS, Encoding.Default);
                    SW.WriteLine(byte_out); //запишем выходной файл
                    byte_out = "";//и очистим переменную
                    SW.Close();
                }
                if (i1 == 118) //запишем первый файл
                {
                    FileStream FS = new FileStream(file_out + "_2.asm", FileMode.Create);
                    StreamWriter SW = new StreamWriter(FS, Encoding.Default);
                    SW.WriteLine(byte_out); //запишем выходной файл
                    byte_out = "";//и очистим переменную
                    SW.Close();
